            ...
            <div class="col-12">
              ...
              @if ($ticket->status != 'closed')
                <form class="ticketForm d-inline-block"
                  action="{{ route('admin.support_ticket.close', ['id' => $ticket->id]) }}" method="post">
                  @csrf
                    ...
                    @else
                      ...
                      <div class="single-message">
                        <div class="user-details">
                          <div class="user-img">
                            <img
                              src="{{ !is_null($user->image) ? asset('assets/img/users/' . $user->image) : asset('assets/img/blank-user.jpg') }}"
                              alt="{{ $user->first_name . ' ' . $user->last_name }}">
                          </div>

                          <div class="user-infos">
                            <h6 class="name">{{ $user->first_name . ' ' . $user->last_name }}</h6>
                            <span class="type"><i class="fas fa-user mr-2"></i>{{ __('Customer') }}</span>
                            <span
                              class="badge badge-secondary">{{ $conversation->created_at->format('M d, Y - h:i A') }}</span>
                          </div>
                        </div>
