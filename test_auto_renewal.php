...
echo "=== Testing Auto-Renewal System ===\n\n";
...
foreach ($transactions as $t) {
    echo "   - ID: {$t->id}, Type: {$t->transcation_type}, Status: {$t->payment_status}\n";
}

// 8. Check for subscription logs
echo "\n8. Checking subscription logs...\n";
$logs = SubscriptionLog::where('seller_id', $membership->seller_id)
    ->latest()
    ->take(3)
    ->get();

echo "   Recent subscription logs count: {$logs->count()}\n";
foreach ($logs as $l) {
    echo "   - ID: {$l->id}, Action: {$l->action}, Description: {$l->description}\n";
}

// 9. Check seller balance
echo "\n9. Checking seller balance...\n";
